...
import { bufferFromUpload, setGravePhoto } from "@/server/photos";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = getGrave(id);
    if (!row || row.provider_id !== user.id) {
      throw new ApiError(404, "Hizmet bulunamadı.", "NOT_FOUND");
    }
    const photoUrl = setGravePhoto(user.id, id, await bufferFromUpload(req));
    return ok({ photoUrl, grave: toPublicGrave(getGrave(id)!) }, 201);
  } catch (e) {
    return fail(e);
  }
}
